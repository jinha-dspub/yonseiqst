...
export default function CMSDashboard() {
  ...
  const fetchInstructors = async () => {
    const { data } = await supabase.from("users").select("*").in("role", ["staff", "admin", "superuser"]);
  ...
  const fetchCohortMembers = async (cohortId: string) => {
    ...
    if (data) setCohortMembers(data.map((row: any) => row.users));
  };
  ...
  const fetchCohortInstructors = async (cohortId: string) => {
    ...
    if (data) setCohortInstructors(data.map((row: any) => row.users));
  };
